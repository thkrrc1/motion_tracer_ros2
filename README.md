...
## 1.各種PKGインストール
1. [seed_robot_ros2_pkg](https://github.com/thkrrc1/seed_robot_ros2_pkg/tree/airoa)、 [noid_lifter_mover](https://github.com/thkrrc1/noid_lifter_mover/tree/airoa)のREADMEに従ってプロジェクトをクローンする。
...
フットペダルのudev設定も行う。
リーダーデバイスPCにフットペダルデバイスのみを接続した状態で、下記コマンド等でフットペダルデバイス情報を取得する。
```
udevadm info /dev/input/event12 (フットペダルデバイスがevent12だった場合)
```
```
・出力例
E: SUBSYSTEM=input
E: USEC_INITIALIZED=30562551784
E: ID_INPUT=1
E: ID_INPUT_KEY=1
E: ID_INPUT_KEYBOARD=1
E: ID_BUS=usb
E: ID_MODEL=FootSwitch
E: ID_MODEL_ENC=FootSwitch
E: ID_MODEL_ID=b010
E: ID_SERIAL=PCsensor_FootSwitch
E: ID_VENDOR=PCsensor
E: ID_VENDOR_ENC=PCsensor
E: ID_VENDOR_ID=3553
...
下記を記入する。（idVendor、idProductに上記で取得した情報を記入）
```
SUBSYSTEM=="input", KERNEL=="event*", ATTRS{idVendor}=="3553", ATTRS{idProduct}=="b010", ENV{ID_INPUT_KEYBOARD}=="1", MODE:="0660", GROUP:="input", SYMLINK+="input/foot_pedal"
```
